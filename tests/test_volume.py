...
def test_make_gaussian_blob_shape() -> None:
    blob = make_gaussian_blob((60, 100, 100))
    assert blob.shape == (60, 100, 100)
    assert blob.dtype == np.float32


def test_make_gaussian_blob_values() -> None:
...
def test_numpy_to_vtk_image() -> None:
...
def test_numpy_to_vtk_image_default_spacing() -> None:
    data = np.random.default_rng(42).random((10, 20, 30), dtype=np.float32)
    image = numpy_to_vtk_image(data)
    assert image.GetSpacing() == (1.0, 1.0, 1.0)


def test_numpy_to_vtk_image_custom_spacing() -> None:
    data = np.random.default_rng(42).random((10, 20, 30), dtype=np.float32)
...
def test_create_volume_without_scattering() -> None:
...
def test_create_volume_with_scattering() -> None:
    blob = make_gaussian_blob((10, 20, 20))
    image_data = numpy_to_vtk_image(blob)
    tf = TransferFunction.default_plume()
    color_tf, opacity_tf = tf.to_vtk()
...
def test_create_volume_sample_distance() -> None:
    blob = make_gaussian_blob((10, 20, 20))
    image_data = numpy_to_vtk_image(blob)
    tf = TransferFunction.default_plume()
    color_tf, opacity_tf = tf.to_vtk()
...
@pytest.mark.skipci
def test_render_blob_to_png(tmp_path: Path) -> None:
...
@pytest.mark.skipci
def test_render_blob_to_output_dir() -> None:
